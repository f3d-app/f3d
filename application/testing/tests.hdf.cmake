...
f3d_test(NAME TestExodus DATA disk_out_ref.ex2 PLUGIN hdf ARGS -s --camera-position=-11,-2,-49)
f3d_test(NAME TestExodusExo DATA box.exo PLUGIN hdf ARGS NO_RENDER NO_BASELINE REGEXP "Number of points: 24")
f3d_test(NAME TestExodusG DATA box.g PLUGIN hdf ARGS NO_RENDER NO_BASELINE REGEXP "Number of points: 24")
...
if (VTK_VERSION VERSION_GREATER_EQUAL 9.5.20251109)
  f3d_test(NAME TestPipedVTKHDF DATA blob.vtkhdf PLUGIN hdf ARGS -s --force-reader=VTKHDF PIPED)
endif()
...
f3d_test(NAME TestNoRenderAnimation DATA small.ex2 PLUGIN hdf ARGS --animation-time=0.003 REGEXP "-994.473, 33.9259" NO_RENDER)

# Test animation time clamping
f3d_test(NAME TestAnimationTimeLimitsHigh DATA small.ex2 PLUGIN hdf ARGS --animation-time=10)
f3d_test(NAME TestAnimationTimeLimitsLow DATA small.ex2 PLUGIN hdf ARGS --animation-time=-5)
f3d_test(NAME TestAnimationTimeLimitsHighNoWarning DATA small.ex2 PLUGIN hdf ARGS --animation-time=0.0043001 REGEXP_FAIL "outside of range" NO_RENDER)
f3d_test(NAME TestAnimationTimeLimitsLowNoWarning DATA small.ex2 PLUGIN hdf ARGS --animation-time=-0.000001 REGEXP_FAIL "outside of range" NO_RENDER)

# Test if negative range is respected when loading a file without specifying the animation time
f3d_test(NAME TestTimeRangeLessThanZeroNoAnimationTime DATA negative_range_animated.e PLUGIN hdf ARGS -s)

# Test if the animation-time works when set to zero and time range[0] is less than zero
f3d_test(NAME TestTimeRangeLessThanZeroWithAnimationTime DATA negative_range_animated.e PLUGIN hdf ARGS -s --animation-time=0)
...
if (NOT F3D_PLUGINS_STATIC_BUILD AND BUILD_SHARED_LIBS)
  ...
  f3d_test(NAME TestPluginName DATA disk_out_ref.ex2 PLUGIN hdf ARGS --verbose NO_BASELINE REGEXP "Loaded plugin hdf from")

  # Test --load-plugins with a full path plugin
  f3d_test(NAME TestPluginFullPath DATA disk_out_ref.ex2 ARGS --verbose --load-plugins "${CMAKE_LIBRARY_OUTPUT_DIRECTORY}/${CMAKE_SHARED_MODULE_PREFIX}f3d-plugin-hdf${CMAKE_SHARED_MODULE_SUFFIX}" NO_BASELINE REGEXP "Loaded plugin hdf from" LABELS "plugin;hdf")
endif()
