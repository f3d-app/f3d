...
f3d_test(NAME TestVTR DATA RectGrid2.vtr ARGS --coloring-mode=scivis --roughness=1)
...
f3d_test(NAME TestDicom DATA IM-0001-1983.dcm ARGS --coloring-mode=scivis --roughness=1)
f3d_test(NAME TestMHD DATA HeadMRVolume.mhd ARGS --coloring-mode=scivis --roughness=1)
f3d_test(NAME TestTIFF DATA f3d.tif ARGS --coloring-mode=direct --up=-Y)
...
f3d_test(NAME TestHDR DATA shanghai_bund_1k.hdr ARGS --camera-orthographic)
if(F3D_MODULE_WEBP)
...
f3d_test(NAME TestGLTFImporterWithAnimation DATA BoxAnimated.gltf ARGS --animation-time=2 --animation-progress UI)
...
f3d_test(NAME TestSPLAT DATA small.splat ARGS -o --coloring-mode=direct --up=-Y --point-sprites-absolute-size --point-sprites-size=1)

# Sort (CPU) isn't supported in GLES because the mapper relies on geometry shaders
f3d_test(NAME TestSPLATSortCPU DATA small.splat ARGS -o --coloring-mode=direct --point-sprites=gaussian --up=-Y --point-sprites-absolute-size --point-sprites-size=1 --blending=sort_cpu --camera-position=2,0,0 SKIP_GLES)
f3d_test(NAME TestSPZ DATA hornedlizard_small_d0.spz ARGS --coloring-mode=direct --point-sprites-absolute-size --point-sprites-size=1)

set(_splat_args --coloring-mode=direct --up=-Y --point-sprites-absolute-size --point-sprites-size=1 --camera-position=-2.00335,1.09654,-0.459485 --camera-focal-point=-0.796712,2.22795,0.705742)
...
f3d_test(NAME TestQuakeMDLActorCollection DATA zombie.mdl ARGS --coloring-mode=scivis)
...
if(VTK_VERSION VERSION_GREATER_EQUAL 9.4.20250501)
  f3d_test(NAME TestPipedPLY DATA suzanne.ply PIPED_READER PLYReader PIPED)
  f3d_test(NAME TestPipedQuakeMDL DATA zombie.mdl PIPED_READER QuakeMDL PIPED)
  f3d_test(NAME TestPipedSPLAT DATA small.splat ARGS -o --coloring-mode=direct --up=-Y --point-sprites-absolute-size --point-sprites-size=1 PIPED_READER Splat PIPED)
  f3d_test(NAME TestPipedSPZ DATA hornedlizard_small_d0.spz ARGS --coloring-mode=direct --point-sprites-absolute-size --point-sprites-size=1 PIPED_READER SPZ PIPED)
...
if(VTK_VERSION VERSION_GREATER_EQUAL 9.5.20250923)
  f3d_test(NAME TestPipedGLTFImporter DATA f3d.glb PIPED_READER GLB PIPED)
  f3d_test(NAME TestPipedGLTFSkin DATA SimpleSkin.gltf PIPED_READER GLTF PIPED)
endif()

if(VTK_VERSION VERSION_GREATER_EQUAL 9.5.20250925)
  f3d_test(NAME TestPiped3DSImporter DATA iflamigm.3ds ARGS --up=+Z PIPED_READER 3DS PIPED)
endif()
...
if(VTK_VERSION VERSION_GREATER_EQUAL 9.5.20251109)
  f3d_test(NAME TestPipedPTS DATA samplePTS.pts PIPED_READER PTS PIPED)
endif()

if(VTK_VERSION VERSION_GREATER_EQUAL 9.5.20251208)
  f3d_test(NAME TestPipedCityGML DATA Part-4-Buildings-V4-one.gml PIPED_READER CityGML PIPED)
endif()

if(VTK_VERSION VERSION_GREATER_EQUAL 9.5.20251210)
  f3d_test(NAME TestPipedDicom DATA IM-0001-1983.dcm ARGS --coloring-mode=scivis --roughness=1 PIPED_READER DICOM PIPED)
  f3d_test(NAME TestPipedPNG DATA world.png ARGS --camera-orthographic PIPED_READER PNG PIPED)
  f3d_test(NAME TestPipedJPG DATA world.jpg ARGS --camera-orthographic PIPED_READER JPEG PIPED)
  f3d_test(NAME TestPipedBMP DATA albedo.bmp ARGS --camera-orthographic PIPED_READER BMP PIPED)
  f3d_test(NAME TestPipedTGA DATA world.tga ARGS --camera-orthographic PIPED_READER TGA PIPED)
  f3d_test(NAME TestPipedHDR DATA shanghai_bund_1k.hdr ARGS --camera-orthographic PIPED_READER HDR PIPED)
  if(F3D_MODULE_WEBP)
    f3d_test(NAME TestPipedWebP DATA image.webp ARGS --camera-orthographic PIPED_READER WebP PIPED)
  endif()
  if(F3D_MODULE_EXR)
    f3d_test(NAME TestPipedEXR DATA Rec709.exr ARGS --camera-orthographic PIPED_READER EXR PIPED)
  endif()
endif()

if(VTK_VERSION VERSION_GREATER_EQUAL 9.6.20260103)
  f3d_test(NAME TestPipedOBJ DATA world.obj PIPED_READER OBJ PIPED) # MTL and Textures not loaded
  f3d_test(NAME TestPipedVTK DATA cow.vtk PIPED_READER VTKLegacy PIPED)
endif()

if(VTK_VERSION VERSION_GREATER_EQUAL 9.6.20260106)
  f3d_test(NAME TestPipedVTR DATA RectGrid2.vtr ARGS --coloring-mode=scivis --roughness=1 PIPED_READER VTKXMLVTR PIPED)
  f3d_test(NAME TestPipedVTU DATA dragon.vtu PIPED_READER VTKXMLVTU PIPED)
  f3d_test(NAME TestPipedVTP DATA cow.vtp PIPED_READER VTKXMLVTP PIPED)
  f3d_test(NAME TestPipedVTS DATA bluntfin.vts PIPED_READER VTKXMLVTS PIPED)
  f3d_test(NAME TestPipedVTICell DATA waveletMaterial.vti ARGS -s --coloring-array=Material -c --roughness=1 PIPED_READER VTKXMLVTI PIPED)
endif()
...
if(NOT F3D_MACOS_BUNDLE)
  ...
  f3d_test(NAME TestThumbnailConfigFileVTU DATA dragon.vtu CONFIG thumbnail_build LONG_TIMEOUT DEFAULT_HDRI)
  f3d_test(NAME TestThumbnailConfigFileVTI DATA vase_4comp.vti CONFIG thumbnail_build LONG_TIMEOUT DEFAULT_HDRI)
  f3d_test(NAME TestThumbnailConfigFileSTL DATA suzanne.stl CONFIG thumbnail_build LONG_TIMEOUT DEFAULT_HDRI)
  f3d_test(NAME TestThumbnailConfigFilePLY DATA suzanneRGBA.ply CONFIG thumbnail_build LONG_TIMEOUT DEFAULT_HDRI SKIP_GLES)
  f3d_test(NAME TestThumbnailConfigFileTGA DATA world.tga CONFIG thumbnail_build LONG_TIMEOUT DEFAULT_HDRI)
  if(VTK_VERSION VERSION_GREATER_EQUAL 9.4.20250118)
    f3d_test(NAME TestThumbnailConfigFileHDR DATA shanghai_bund_1k.hdr CONFIG thumbnail_build LONG_TIMEOUT DEFAULT_HDRI)
    if(F3D_MODULE_EXR)
      f3d_test(NAME TestThumbnailConfigFileEXR DATA Rec709.exr CONFIG thumbnail_build LONG_TIMEOUT DEFAULT_HDRI)
    endif()
  endif()
  if(F3D_MODULE_WEBP)
