...
int test_pseudo_unit_test()
{
  ...
  f3d_test_check_double(&test, "double equality passes", 1.0, 1.0);
  ...
  f3d_test_check_double(&inner2, "double outside tol fails", 1.5, 1.0);
  int int_double_fail_detected = (inner2.fail_count == 2 && inner2.pass_count == 0);

  f3d_test_t boundary;
  f3d_test_init(&boundary);
  double tol = 128 * DBL_EPSILON;
  f3d_test_check_double(&boundary, "exactly at tolerance boundary fails", 1.0 + tol, 1.0);
