...
|      model.color.rgb       |  vector\<double\><br>optional<br>render   | Set a _color_ on the geometry. Multiplied with the `model.color.texture` when present. Model specified by default.                                                                                                                                   |       \-\-color        |
...
|   model.scivis.colormap    | vector\<double\><br>\<inferno\><br>render | Set a _custom colormap for the coloring_.<br>This is a list of colors in the format `val1,red1,green1,blue1,...,valN,redN,greenN,blueN`<br>where all values are in the range (0,1).                                                                  |      \-\-colormap      |
...
|    render.effect.anti_aliasing     |          bool<br>false<br>render          | Enable _anti-aliasing_. This technique is used to reduce aliasing, implemented using FXAA.                                                                                |                          \-\-anti-aliasing                           |
...
|          render.hdri.file          |       string<br>optional<br>render        | Set the _HDRI_ image that can be used for ambient lighting and skybox.<br>Valid file format are hdr, exr, png, jpg, pnm, tiff, bmp.<br>If not set, a default is provided. |                            \-\-hdri-file                             |
...
|      render.background.color       | vector\<double\><br>0.2,0.2,0.2<br>render | Set the window _background color_.<br>Ignored if a _hdri_ skybox is used.                                                                                                 |                         \-\-background-color                         |
