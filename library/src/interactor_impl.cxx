...
interactor& interactor_impl::initCommands()
{
  ...
  this->addCommand("lock_axis", 
    ...
    { 
      ...
      std::string_view axis = args[0];

      auto& lock = this->Internals->Options.interactor.lock_axis;
      std::string_view prevLocked = "none";
      if (lock.x)
      {
        prevLocked = "x";
      }
      ...
      if (axis == "x" && axis != prevLocked)
      {
        lock.x = true;
      }
      else if (axis == "y" && axis != prevLocked)
      {
        lock.y = true;
      }
      else if (axis == "z" && axis != prevLocked)
      {
        lock.z = true;
      }
    });
...
interactor& interactor_impl::initBindings()
{
  ...
  this->addBinding({mod_t::CTRL, "X"}, "lock_axis x", "Scene", std::bind(docTgl, "Toggle x axis lock", std::cref(opts.interactor.lock_axis.x)));
  this->addBinding({mod_t::CTRL, "Y"}, "lock_axis y", "Scene", std::bind(docTgl, "Toggle y axis lock", std::cref(opts.interactor.lock_axis.y)));
  this->addBinding({mod_t::CTRL, "Z"}, "lock_axis z", "Scene", std::bind(docTgl, "Toggle z axis lock", std::cref(opts.interactor.lock_axis.z)));
