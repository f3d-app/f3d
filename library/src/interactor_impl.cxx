...
/*  auto complCommandActions = [&](const std::vector<std::string>& args)
  ...
  auto complEt = [&](const std::vector<std::string>& args)
  { return complNames(args, this->Internals->Options.getNames()); };
...
//    "azimuth_camera value: tilt the camera right or left");
  ...
  this->addCommand(
    ...
    { this->Internals->IncreaseOpacity(false); });
  //, "increase opacity");

  this->addCommand(
    "decrease_opacity", [&](const std::vector<std::string>&)
    { this->Internals->IncreaseOpacity(true); });
  //, "decrease opacity");
