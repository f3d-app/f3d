...
PYBIND11_MODULE(pyf3d, module)
{
  ...
  options //
    ...
    .def("__len__", [](f3d::options& opts) { return opts.getSetNames().size(); })
    .def(
      "__iter__",
      [](f3d::options& opts)
      {
        const auto names = py::cast(opts.getSetNames()); // won't work without cast
        return make_iterator(names);
      },
      py::keep_alive<0, 1>())
    .def("keys", &f3d::options::getSetNames) // to do `dict(options)`
