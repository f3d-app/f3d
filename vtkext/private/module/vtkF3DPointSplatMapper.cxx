...
void vtkF3DSplatMapperHelper::BuildBufferObjects(vtkRenderer* ren, vtkActor* act)
{
...
#if VTK_VERSION_NUMBER < VTK_VERSION_CHECK(9, 4, 20250508)
  ...
  if (sh1m1 && sh10 && sh1p1)
  {
    ...
    std::cout << "Spherical harmonics texture size: " << width << "x" << height << std::endl;
  ...
  std::map<vtkShader::Type, vtkShader*> shaders, vtkRenderer* ren, vtkActor* actor)
{
  ...
  {
    // shStr << "  vec3 cameraDirection = normalize(inverse(MCVCMatrix) * vec4(0.0, 0.0, -1.0,
    // 0.0)).xyz;\n";
