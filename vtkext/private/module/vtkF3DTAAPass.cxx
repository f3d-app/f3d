...
void vtkF3DTAAPass::Render(const vtkRenderState* state)
{
  ...
  if (this->MotionVectorTexture == nullptr)
  {
    ...
    this->MotionVectorTexture->SetInternalFormat(GL_RG32F);
    this->MotionVectorTexture->SetDataType(GL_FLOAT);
  ...
  this->DepthTexture->Resize(this->ViewPortSize[0], this->ViewPortSize[1]);
  ...
  if (this->MovementBlendingMode == 0)
  {
    this->RenderDelegate(state, this->ViewPortSize[0], this->ViewPortSize[1], this->ViewPortSize[0],
      this->ViewPortSize[1], this->FrameBufferObject, this->ColorTexture);
  }
  else if (this->MovementBlendingMode == 1)
  {
  ...
  if (!this->QuadHelper)
  {
    ...
    else if (this->MovementBlendingMode == 1)
    {
      ...
      vtkShaderProgram::Substitute(TAAResolveFS, "//VTK::FSQ::Impl",
        ...
        "   vec4 c = texture(colorTexture, texCoord + offset);\n"
        "   minC = min(minC, c);\n"
        "   maxC = max(maxC, c);\n"
        " }\n"
        "}\n"
        "vec4 clampedHistory = clamp(history, minC, maxC);\n"
        "vec4 outColor = mix(current, clampedHistory, blendFactor);\n"
  ...
  if (this->MovementBlendingMode == 1)
  {
