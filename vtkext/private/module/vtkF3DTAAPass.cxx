...
void vtkF3DTAAPass::Render(const vtkRenderState* state)
{
  ...
  if (!this->QuadHelper)
  {
    ...
    vtkShaderProgram::Substitute(TAAResolveFS, "//VTK::FSQ::Impl",
      ...
      "vec3 c0 = texture(colorTexture, texCoord + vec2( texelSize.x, 0.0)).xyz;\n"
      "vec3 c1 = texture(colorTexture, texCoord + vec2(-texelSize.x, 0.0)).xyz;\n"
      "vec3 c2 = texture(colorTexture, texCoord + vec2(0.0,  texelSize.y)).xyz;\n"
      "vec3 c3 = texture(colorTexture, texCoord + vec2(0.0, -texelSize.y)).xyz;\n"
      "vec3 boxMin = min(current.xyz, min(c0, min(c1, min(c2, c3))));\n"
      "vec3 boxMax = max(current.xyz, max(c0, max(c1, max(c2, c3))));\n"
      "history.xyz = clamp(history.xyz, boxMin, boxMax);\n"
      "vec3 outputRGB = mix(current.rgb, history.rgb, blendFactor);\n"
      "float outputAlpha = current.a;\n"
      "gl_FragData[0] = vec4(outputRGB, outputAlpha);\n"
      "//VTK::FSQ::Impl");
    this->QuadHelper =
