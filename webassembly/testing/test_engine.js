...
const settings = {
  run: (Module) => {
    ...
    utils.assert(readerOptions.includes("QuakeMDL.skin_index"), "Cannot find reader option");
    ...
    readersInfo.forEach((reader) => {
      utils.assert(reader.extensions.length > 0, "Cannot find reader extensions for " + reader.name);
      utils.assert(reader.mimeTypes.length > 0, "Cannot find reader mime types for " + reader.name);
      utils.assert(reader.hasSceneReader || reader.hasGeometryReader, "Cannot find reader scene or geometry support for " + reader.name);
    });

    utils.assert(readersInfo.map((reader) => reader.name).includes("GLTF"), "Cannot find GLTF reader");
  },
};
