...
const settings = {
  run: async (Module) => {
    ...
    utils.assert(Module.Image.getSupportedFormats().includes(".png"), "supported formats");
    ...
    utils.assert(img.channelType === Module.ImageChannelType.BYTE, "channel type");
    ...
    utils.assert(
      content[0] === 255 && content[1] === 0 && content[2] === 0 && content[3] === 255 && content[4] === 0 && content[5] === 0 && content[6] === 255 && content[7] === 255,
    ...
    utils.assert(pngBuffer[0] === 137 && pngBuffer[1] === 80 && pngBuffer[2] === 78 && pngBuffer[3] === 71, "png buffer header");
    ...
    utils.assert(utils.numArrayEquals(normalizedPixel, [1, 0, 0, 1]), "normalized pixel");

    img.setMetadata("Author", "John Doe");
    utils.assert(img.allMetadata().includes("Author"), "allMetadata includes Author");
    utils.assert(
